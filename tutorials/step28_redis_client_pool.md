...
本 Step 不实现具体业务 cache。`OnlineStatusCache` 在 Step 29，未读计数和登录失败限制在 Step 30。
...
tests/cache/redis_client_pool_test.cpp
tutorials/step28_redis_client_pool.md
```
...
`liteim_cache` 从 header-only 接口目标升级为静态库，并通过 `pkg-config hiredis` 链接 hiredis。

## 3. RedisClient.hpp 接口说明
...
## RedisClient / RedisPool 的作用场景和运行流程
...
第一版只需要可靠、可测试、可重连的阻塞 Redis client 和 pool。
