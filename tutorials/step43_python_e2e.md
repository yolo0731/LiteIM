...
- 不解决动态端口、多 server 并行 E2E、测试数据清理和压测统计；这些留给后续测试/CI/bench 细化。
...
seed 里的 `alice` / `bob` / `mira_bot` 使用 dev hash，不一定能通过 `AuthService` 的 PBKDF2 校验。E2E 测试统一先注册唯一用户名，再登录真实密码，避免依赖 seed 密码。
...
> 为什么不用 pytest？第一版为了降低依赖，只用 Python 标准库 `unittest`。后续 CI 或更复杂断言需要 fixture/marker 时，可以再引入 pytest。
