...
Step 44 的压测工具不是为了制造夸张数据，而是为了给后续优化、CI 手动验证和最终简历报告提供可复现的测量入口。
...
- 不提前实现 Step 45 的 gMock、ASan/UBSan 或 Step 46 的 CI。
...
> 为什么不把压测放进默认 CI？因为压测依赖本机资源、Docker MySQL/Redis 和时间窗口，结果不可稳定断言。CI 可以保留构建和单测，真实压测放手动 workflow 或本地脚本。
