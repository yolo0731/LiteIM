...
TEST(ConcurrencyInterfaceTest, ThreadPoolHeaderIsSelfContained) {
    ...
    static_assert(std::is_same_v<decltype(&ThreadPool::submit), liteim::Status (ThreadPool::*)(Task)>);
    static_assert(std::is_same_v<decltype(&ThreadPool::stop), void (ThreadPool::*)() noexcept>);
    static_assert(std::is_same_v<decltype(&ThreadPool::workerCount), std::size_t (ThreadPool::*)() const noexcept>);
    static_assert(std::is_same_v<decltype(&ThreadPool::pendingTaskCount), std::size_t (ThreadPool::*)() const>);
    static_assert(std::is_same_v<decltype(&ThreadPool::started), bool (ThreadPool::*)() const noexcept>);
