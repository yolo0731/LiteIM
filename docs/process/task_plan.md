...
| Step 54 client_msg_id idempotent sends | pending | Add sender-provided client message id for retry-safe private sends, database uniqueness, and duplicate-send response reuse. |
| Step 55 private delivery ACK | pending | Add receiver-side private-message delivery ACK for online `PrivateMessagePush` without starting group-wide member ACK fanout. |
| Step 56 ThreadPool queue cap and limiter verification | pending | Add a bounded business task queue / rejection path and verify real peer-IP login limiter behavior after the post-review IP fix. |
