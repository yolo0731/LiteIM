...
| Phase | Status | Notes |
| --- | --- | --- |
| Step 47 Qt login and register window | done | Added `AuthController`, `LoginWindow`, `RegisterDialog`, and a testable `ClientApp` login-success bridge that opens `MainWindow`. RED first failed on missing `liteim_client/AuthController.hpp`; GREEN passed Qt Step46/Step47 CTest, Qt offscreen startup, default unit CTest 311/311, full Docker-backed default CTest 381/381, diff check, and tutorial scans. Default `build` remains free of Qt dependencies; Qt UI tests run with `QT_QPA_PLATFORM=offscreen` and the existing Qt CTest `LD_LIBRARY_PATH` override. |
