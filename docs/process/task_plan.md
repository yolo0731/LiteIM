...
| Phase | Status | Notes |
| --- | --- | --- |
| Post-Step58 reliability and cleanup follow-up | done | User confirmed the larger review items must also be fixed. Implemented synchronous online-push fallback, duplicate `client_msg_id` receiver-side delivery-state repair, AuthDao removal, MessageDao/MySqlStorage save-path consolidation, reverse pending friend-request rejection, and current-facing doc sync. Verification passed targeted RED/GREEN suites, `-Wall -Wextra -Wpedantic` review build, default CTest 419/419, Qt CTest 8/8, tutorial heading check, stale-route/temp-file scans, and diff whitespace check. |
| Post-Step58 final project audit | done | Reviewed code, Markdown, generated artifacts, and unused-surface risks after the Step58 first-version closeout. Found and fixed one real code/doc/protocol mismatch: `LogoutRequest` / `LogoutResponse` were defined and classified but not wired through `AuthService` / CLI / E2E. Verification passed RED/GREEN logout coverage, `-Wall -Wextra -Wpedantic` review build, default full CTest 414/414, Qt CTest 8/8, tutorial heading check, diff whitespace check, stale-path scan, temporary-file scan, and ignored-artifact cleanup. |
...
| Step 57 friend request and private permission | done | Replaced direct add-friend semantics with pending friend requests, added accept/reject protocol and storage flow, kept `friendships` as accepted-only rows, and made private chat require accepted friendship. Updated E2E and benchmark setup to establish accepted friendship before private sends. Verification passed targeted Step57/benchmark suite 71/71, full default CTest 411/411, schema migration, benchmark smoke, diff whitespace check, and tutorial/noise scans. |
| Step 58 final README/showcase materials | done | Refreshed README, delivery semantics, known limitations, interview notes, 2026-05-20 benchmark report, Qt showcase screenshot, and Step58 tutorial after ACK/idempotency/limiter/friend-permission hardening. This is the final LiteIM first-stage showcase closeout. |
...
| Step 41 CLI test client | done | Added `client_cli/` with `liteim_client_cli` and `liteim_cli`. The CLI connects to `127.0.0.1:9000` by default with `--host` / `--port` overrides, reads interactive/stdin commands, sends TLV requests for register/login/logout/friend/private/group/history/offline/heartbeat flows, prints decoded responses and push packets, and sends a background heartbeat every 30 seconds. Added command parsing, packet description, blocking TCP protocol client, targeted CLI tests, README/tutorial/planning docs, full build passed, ClientCli tests passed 5/5 at Step41 and later grew with reliability/logout coverage, full CTest passed with local Docker MySQL/Redis, diff check passed, stale-route scans passed, tutorial scans passed, and bounded server smoke reached listening state then exited through signalfd SIGTERM. No TUI, local persistence, reconnect, Python E2E, benchmark, Qt, schema, Redis key, or PersonaAgent behavior introduced. |
...
| Step 34 AuthService register, login, and logout | done | Added `AuthService` with RED/GREEN tests, PBKDF2-HMAC-SHA256 password hashing, register/login handlers over `IStorage` / `ICache` / `OnlineService`, and server runtime wiring for MySQL / Redis / AuthService. Post-Step58 audit wired the already-defined `LogoutRequest` / `LogoutResponse` into `AuthService`, CLI, and E2E so logout now unbinds the current session and clears online state. Register/login/logout run in the business thread pool through `MessageRouter`; successful login clears failure counts, writes Redis online state, and binds `SessionManager`. Full build, full CTest, diff check, and server smoke passed at their respective checkpoints. |
...
| Step 25 UserDao | done | Added `UserDao` for users-table data access, `ErrorCode::AlreadyExists`, structured duplicate-key conversion through `PreparedStatement::lastErrorNumber()`, and Docker-backed tests for create, duplicate username, find by username/id, and missing user. The old AuthDao teaching helper was removed during the Post-Step58 cleanup; current AuthService uses `UserDao::findUserByUsername()`. No service, session, Redis, message DAO, or runtime server integration was introduced in the original Step. |
...
- Step 25 `UserDao` is complete; the old AuthDao helper was removed during Post-Step58 cleanup.
...
- Step 34 `AuthService register, login, and logout` is complete.
