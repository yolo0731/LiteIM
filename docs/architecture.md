...
## Step 13：MessageRouter 心跳基础

Step 13 已经实现 `MessageRouter`：

- 头文件：`include/liteim/service/MessageRouter.hpp`
- 实现文件：`src/service/MessageRouter.cpp`

`MessageRouter` 是第一版业务层分发器。它不处理 socket 读写，也不管理连接生命周期，而是接收 `Session` 已经解出来的 `Packet`：
...
- `HEARTBEAT_REQ`：返回 `HEARTBEAT_RESP`。
- 其他未知 `msg_type`：返回 `ERROR_RESP`。

响应包会保留请求包的 `seq_id`。这样客户端后续可以根据同一个 `seq_id` 把请求和响应对应起来。

`MessageRouter` 的边界很明确：
