...
- `isRequestType()` / `isResponseType()` / `isPushType()`：区分请求、响应和服务端推送，为后续 `MessageRouter`、Qt 客户端和 PersonaAgent BotClient 做准备。
...
- 100 段是注册、登录、登出。
- 200 段是好友申请、接受、拒绝、好友列表相关请求和响应。
- 300 段是私聊请求、响应和服务端推送。
- 400 段是群组和群聊消息。
- 500 段是离线消息、离线 ACK 和历史消息。
- 600 段暂不使用，后续扩展必须重新评估协议兼容性。
...
- 用户和认证字段包括 `Username`、`Password`、`UserId`、`Nickname`、`Token`、`SessionId`。当前第一版登录只返回 `SessionId`，`Token` 是为后续 access-token 认证预留的协议字段。
- 好友字段包括 `FriendId`、`TargetUserId`、`OnlineStatus`、`FriendRequestStatus`。
- 群组字段包括 `GroupId`、`GroupName`。
- 消息字段包括 `ConversationType`、`ConversationId`、`MessageId`、`MessageText`、`SenderId`、`ReceiverId`、`TimestampMs`、`Offset`、`Limit`、`UnreadCount`、`DeliveryStatus`、`ClientMessageId`。
...
`MessageType` 是 Packet 级路由键，决定这是登录、私聊、群聊、历史、心跳还是错误响应；`TlvType` 是 body 字段键，决定用户名、密码、用户 ID、消息文本等字段怎么读。LiteIM server、后续 Qt client 和后续 PersonaAgent BotClient 必须共享同一套数字值。
...
MessageType
    ↓
request 分组：Heartbeat / Login / Logout / PrivateMessage / ...
    ↓
response 分组：HeartbeatResponse / LoginResponse / LogoutResponse / ...
    ↓
push 分组：PrivateMessagePush / GroupMessagePush
    ↓
上层获得轻量分类结果
```
...
Step57 之后，好友关系不再由 `AddFriendRequest` 直接创建。Alice 发送 `AddFriendRequest(TargetUserId=1002)` 只会创建 pending 申请；Bob 发送 `AcceptFriendRequest(TargetUserId=1001)` 后，MySQL 才写入双向 `friendships` 行。Bob 也可以发送 `RejectFriendRequest(TargetUserId=1001)`，响应会带 `FriendRequestStatus = 2`。
...
- 类型值足够多，未来可以继续扩展。
- 后续跨语言实现 PersonaAgent BotClient 时，也可以按整数值映射消息类型。
...
600-699  reserved for future extensions
900      error response
```
...
Push 既不是 request，也不是 response，但它也不能和 `Unknown` 混在一起。后续 Qt 客户端和 PersonaAgent BotClient 收到 Push 时，要走会话列表刷新、消息气泡追加、桌面通知或外部 Agent 消息处理逻辑；收到未知类型时应该走错误日志或忽略策略。
...
enum class TlvType : std::uint16_t {
    ...
    UserId = 3,
    Nickname = 4,
    Token = 5,      // reserved for future access-token auth
    SessionId = 6,
...
Body:
  ...
  TLV(CLIENT_MESSAGE_ID, "cli-msg-1")  # 可选
...
> 我在协议层先定义了 `MessageType` 和 `TlvType`。`MessageType` 进入 Packet header，用来区分登录、私聊、群聊、离线消息、历史消息、心跳和错误响应；`TlvType` 用在 body 里，用 TLV 方式表达用户名、用户 ID、消息文本、群组 ID 等字段。请求、响应和推送消息分别用 `isRequestType()`、`isResponseType()` 和 `isPushType()` 显式分类，后续路由层和客户端都能区分业务请求、请求结果和服务端主动推送。

重点表达：

- 协议类型定义是跨服务端、当前 CLI、后续 Qt 和后续 PersonaAgent BotClient 的共同契约。
