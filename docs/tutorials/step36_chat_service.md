...
- Step 36 当时不做好友关系校验；Step 57 已把当前运行时收紧为 accepted 好友才能私聊。
...
Step 36 当时不会，只要求接收方用户存在。当前代码在 Step 57 后已经会检查 `IStorage::areFriends(sender_id, receiver_id)`，只有 accepted 好友关系才能发送私聊。
