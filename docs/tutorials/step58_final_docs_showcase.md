...
Step58 是 LiteIM 第一阶段的最终展示材料收口：README、架构图、线程模型、协议/数据模型摘要、Qt 截图、压测报告、面试说明和已知限制统一反映 Step53-57 以及后续第一版审计修复的可靠性、安全和权限 hardening 结果。

本 Step 本身不新增服务端业务行为。当前第一版服务端能力包括：离线 ACK、`client_msg_id` 幂等、重复请求按 delivery 状态修复 receiver-side 投递、私聊 delivery ACK、同步在线 push 失败 fallback、业务线程池队列上限、真实 peer IP 登录限流验证、好友申请和私聊好友权限。
...
- `ClientMessageId`：发送方幂等键，解决网络重试导致重复消息；重复请求会返回已有消息，并确保缺失的离线 fallback row 可以补齐。
- `DeliveryAckRequest`：接收方收到在线私聊 push 后回 ACK，表示 delivered。
- `OfflineMessagesAckRequest`：客户端拉取离线消息后批量 ACK，ACK 后才 mark delivered。
- `Session::sendPacket()`：同步判定输出高水位等不可排队失败时返回错误，私聊可补 offline fallback；已经排入 I/O loop 后再断开的异步失败仍依赖 ACK/重试边界说明。
...
```text
接收方在线但输出高水位/不可排队
  -> Session::sendPacket() 返回 ResourceExhausted
  -> ChatService 保存 offline_messages fallback
  -> 接收方下次 OfflineMessagesRequest 仍可拉取
```
