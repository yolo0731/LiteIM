# Step 44：测试覆盖、gMock 和 ASan/UBSan

## 0. 本 Step 结论

- 目标：Step 44 扩充关键模块测试覆盖，引入 gMock 边界测试，并增加 ASan/UBSan 构建入口。
...
Step 34-43 已经把 LiteIM 的业务服务、E2E 和压测工具串起来，但测试仍有几个缺口：
...
Step 44 的重点不是改业务语义，而是把现有语义用更硬的测试边界固定下来。
...
- 让仓库级 GitHub Actions 复用这些标签和 sanitizer 入口，作为 Step 44 验证能力的自动化承载。
...
- 不把 CI 拆成独立 Step 45；CI 只是仓库基础设施。
...
| `.github/workflows/ci.yml` | 新增 | 复用 Step 44 CTest 标签和 sanitizer 入口，在 GitHub runner 上跑 unit、integration、ASan/UBSan |
| `README.md` / planning 文件 | 更新 | 记录 Step 44 命令、标签、边界和验证结果 |
...
`.github/workflows/ci.yml` 的三条 job 不重新定义测试范围：`unit` 只跑 `ctest -L unit`，`integration` 先启动 Docker MySQL/Redis 再跑 `ctest -L integration`，`sanitizers` 只打开 `LITEIM_ENABLE_SANITIZERS=ON` 后跑 full CTest。也就是说，CI 的稳定性来自 Step 44 已经整理好的标签和构建入口，而不是另起一套测试系统。
...
> Step 44 主要补测试工程能力。我给 service 层引入 gMock 边界测试，验证 Auth、Chat、Group、History 对 storage/cache/online 的调用顺序和参数；同时给协议解码、TLV、线程池、定时器补边界用例，并通过 CTest 标签把单元、MySQL、Redis、Docker、E2E 测试分开。最后增加 ASan/UBSan 构建入口，并让 GitHub Actions 复用这些入口自动跑验证。
...
> 为什么 CI 不单独做成 Step？因为 CI 不改变 LiteIM 的服务端协议、存储语义或客户端功能，它只是把 Step 44 整理出的验证入口放到 GitHub 的干净 runner 上自动执行。项目路线继续从 Step 45 进入 Qt 客户端。

## 10. 面试常见追问

### 为什么不把所有 service fake 测试都换成 gMock？

fake 测试更适合验证完整业务结果，gMock 更适合验证依赖调用边界。全部替换会增加维护成本，也容易让测试过度绑定实现细节。Step 44 只在边界最关键的位置使用 mock。
...
Step 44 负责把验证能力标准化：测试可用 CTest 标签筛选，sanitizer 可用 CMake 选项开启。CI 只是调用这些标准入口，所以它属于 Step 44 验证体系的自动化补充，不需要占用新的功能 Step。
