...
- 目标：Step 21 的目标不是连接 MySQL 或 Redis，而是先把业务层需要的“存储能力”和“缓存能力”抽象出来。
- 前置依赖：依赖 Step 0-20 已建立的工程、协议或运行时基础。
- 主要交付：`定义 IStorage / ICache 抽象` 的文件变化、接口契约、运行流程、测试和面试表达。
...
- 聚焦 `定义 IStorage / ICache 抽象` 这一层的当前交付，把前置能力接成可编译、可测试的模块。
...
| 文件 | 变化 | 作用 |
| --- | --- | --- |
| `include/liteim/storage/StorageTypes.hpp` | 新增 | 承载本 Step 对应代码、测试或文档变化 |
| `include/liteim/storage/IStorage.hpp` | 新增 | 承载本 Step 对应代码、测试或文档变化 |
| `include/liteim/cache/CacheTypes.hpp` | 新增 | 承载本 Step 对应代码、测试或文档变化 |
| `include/liteim/cache/ICache.hpp` | 新增 | 承载本 Step 对应代码、测试或文档变化 |
| `tests/storage/storage_interface_test.cpp` | 新增 | 承载本 Step 对应代码、测试或文档变化 |
| `tests/cache/cache_interface_test.cpp` | 新增 | 承载本 Step 对应代码、测试或文档变化 |
| `docs/tutorials/step21_storage_cache_interfaces.md` | 新增 | 承载本 Step 对应代码、测试或文档变化 |
...
| 风险 | 测试如何覆盖 |
| --- | --- |
| `定义 IStorage / ICache 抽象` 的核心契约只停留在接口说明里 | 用单元测试或集成测试固定 public API、正常路径和错误路径 |
...
Step 21 先把 IM 业务需要的持久化和缓存能力抽象出来。
