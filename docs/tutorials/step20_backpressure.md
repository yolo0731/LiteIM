...
服务端给客户端写数据不能刷新活跃时间。否则私聊 push、系统通知、群聊 push 等出站流量会把沉默客户端误判成活跃连接，削弱 heartbeat timeout。
...
| `docs/tutorials/step20_backpressure.md` | 修改 | 承载本 Step 对应代码、测试或文档变化 |
| `README.md` | 修改 | 承载本 Step 对应代码、测试或文档变化 |
| `docs/tutorials/step02_base.md` | 修改 | 承载本 Step 对应代码、测试或文档变化 |
| `docs/tutorials/step14_session.md` | 修改 | 承载本 Step 对应代码、测试或文档变化 |
...
慢客户端回压保护的是输出方向：客户端或网络异常导致长时间不读数据，而服务端仍持续 echo、普通消息 push 或系统通知。非阻塞 `write()` 写不完时，剩余字节进入 `Session::output_buffer_`；超过高水位就记录 warning 并关闭连接，防止单个慢连接吃掉内存。
