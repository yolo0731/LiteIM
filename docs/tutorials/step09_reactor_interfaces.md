...
- 目标：本 Step 进入 Reactor 的接口层，但还不实现真实 epoll 行为。
- 前置依赖：依赖 Step 0-8 已建立的工程、协议或运行时基础。
- 主要交付：`定义 Reactor 核心接口` 的文件变化、接口契约、运行流程、测试和面试表达。
- 线程/生命周期边界：沿用 LiteIM 当前 owner-loop、RAII、业务线程隔离和抽象依赖规则。
- 范围控制：不实现 `src/net/Epoller.cpp`
...
- 聚焦 `定义 Reactor 核心接口` 这一层的当前交付，把前置能力接成可编译、可测试的模块。
- 明确新增/修改文件、核心接口、运行流程、边界条件和验证方式。
- 保持当前 Step 的实现范围，不把后续路线混入本 Step。

### 本 Step 不做

- 不实现 `src/net/Epoller.cpp`
- 不实现 `src/net/Channel.cpp`
- 不实现 `src/net/EventLoop.cpp`
- 不实现 `epoll_create1()`
- 不实现 `epoll_ctl()`
- 不实现 `epoll_wait()`
- 不实现 `Channel::handleEvent()` 回调分发
- 不实现 `EventLoop::loop()` 阻塞循环
- 不实现 `eventfd` 跨线程唤醒
- 不实现 `Acceptor`
- 不实现 `Session`
- 不实现 `TcpServer`
...
| 文件 | 变化 | 作用 |
| --- | --- | --- |
| `include/liteim/net/Channel.hpp` | 修改 | 承载本 Step 对应代码、测试或文档变化 |
| `include/liteim/net/Epoller.hpp` | 修改 | 承载本 Step 对应代码、测试或文档变化 |
| `include/liteim/net/EventLoop.hpp` | 修改 | 承载本 Step 对应代码、测试或文档变化 |
| `tests/net/channel_header_test.cpp` | 修改 | 承载本 Step 对应代码、测试或文档变化 |
| `tests/net/epoller_header_test.cpp` | 修改 | 承载本 Step 对应代码、测试或文档变化 |
| `tests/net/event_loop_header_test.cpp` | 修改 | 承载本 Step 对应代码、测试或文档变化 |
| `tests/CMakeLists.txt` | 修改 | 承载本 Step 对应代码、测试或文档变化 |
| `src/net/Channel.cpp` | 修改 | 承载本 Step 对应代码、测试或文档变化 |
| `src/net/Epoller.cpp` | 修改 | 承载本 Step 对应代码、测试或文档变化 |
...
- [Channel.hpp](../include/liteim/net/Channel.hpp) 保存 fd、关注事件、实际事件、回调和弱生命周期保护。
- [Epoller.hpp](../include/liteim/net/Epoller.hpp) 保存 epoll fd、事件数组和 fd 到 Channel 的映射。
- [EventLoop.hpp](../include/liteim/net/EventLoop.hpp) 保存 owner 线程、wakeup fd、pending task 队列和 Epoller。
...
| 风险 | 测试如何覆盖 |
| --- | --- |
| `定义 Reactor 核心接口` 的核心契约只停留在接口说明里 | 用单元测试或集成测试固定 public API、正常路径和错误路径 |
...
我把 Reactor 核心拆成三层：Channel 表示 fd 和关注事件，Epoller 只封装 Linux epoll 的注册、删除和等待边界，EventLoop 负责持有 Epoller 并调度活跃 Channel。
