...
- 目标：Step 48 把登录后的空 `MainWindow` 改成常见微信式 IM 三栏主窗口。
...
- 左侧固定功能栏承载消息、联系人、群组和设置。
- 中间区域根据左侧功能切换不同列表。
- 右侧聊天页预留后续消息展示、输入框和会话标题。
- 顶部展示当前用户昵称和在线状态。
- 视觉思路参考常见微信式三栏 IM 交互，但不使用微信品牌、名称、logo、图标、截图或素材。
...
- 新增 `SideBar`，提供四个左侧功能按钮。
- 新增 `ConversationListWidget`，用占位列表表示消息、联系人、群组和设置区域。
...
- 不在 `SideBar` 顶级导航中加入 Agent 分类；后续 PersonaAgent 只作为普通联系人或普通会话出现。

## 3. 文件变化

| 文件 | 变化 | 作用 |
| --- | --- | --- |
| `client_qt/include/liteim_client/ui/SideBar.hpp` | 新增 | 左侧功能导航栏 |
| `client_qt/src/ui/SideBar.cpp` | 新增 | 四个导航按钮和选中状态 |
| `client_qt/include/liteim_client/ui/ConversationListWidget.hpp` | 新增 | 中间列表区域 |
| `client_qt/src/ui/ConversationListWidget.cpp` | 新增 | 根据功能切换占位列表 |
| `client_qt/include/liteim_client/ui/ChatPage.hpp` | 新增 | 右侧聊天页 |
| `client_qt/src/ui/ChatPage.cpp` | 新增 | 顶部用户状态、聊天区和输入占位 |
| `client_qt/include/liteim_client/ui/MainWindow.hpp` | 修改 | 持有三栏子组件 |
| `client_qt/src/ui/MainWindow.cpp` | 修改 | 使用 `QSplitter` 组合三栏并连接切换 |
| `client_qt/resources/qss/app.qss` | 修改 | 三栏布局和导航按钮样式 |
| `client_qt/tests/qt_client_test.cpp` | 修改 | 新增 Step 48 Qt 主窗口测试 |
| `client_qt/src/CMakeLists.txt` | 修改 | 注册三栏主窗口相关源文件 |
| `client_qt/tests/CMakeLists.txt` | 修改 | 注册 `LiteIMQtClient.Step48` |
...
- `section_id` 使用简单字符串：`messages`、`contacts`、`groups`、`settings`。
...
### 3. PersonaAgent 不进入顶级导航

未来 PersonaAgent 仍然是普通 LiteIM 账号。Qt 侧应该把它显示在联系人列表或会话列表里，而不是在 `SideBar` 建一个特殊的 Agent 分类。这样 UI 表达和服务端边界一致：LiteIM C++ 服务端不识别 AI 身份。
...
| 左侧按钮缺失或多出 Agent 顶级入口 | 同一测试检查消息、联系人、群组、设置按钮，并确认没有 `navAgentButton` |
...
> Step 48 我把 Qt 客户端从登录后的空窗口升级成了常见微信式 IM 三栏主界面。左侧 `SideBar` 只放消息、联系人、群组和设置；中间 `ConversationListWidget` 根据左侧选择切换列表区域；右侧 `ChatPage` 展示聊天区占位、当前用户昵称和在线状态。布局用 `QSplitter`，左栏固定，中栏限制宽度，右栏自适应拉伸。PersonaAgent 后续作为普通联系人或会话出现，不是顶级导航分类。
...
### PersonaAgent 是不是特殊 AI 协议？

不是。LiteIM 服务端不会识别 AI 身份，后续 PersonaAgent 也通过普通账号登录和收发消息。Qt 侧不会在 `SideBar` 放 Agent 顶级入口，而是在联系人或会话列表里把它当普通对象展示。
