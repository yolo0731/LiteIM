...
- 目标：本 Step 在 Step 9 的接口基础上实现真正的 Linux epoll 封装。
- 前置依赖：依赖 Step 0-9 已建立的工程、协议或运行时基础。
- 主要交付：`实现 Epoller` 的文件变化、接口契约、运行流程、测试和面试表达。
...
- 聚焦 `实现 Epoller` 这一层的当前交付，把前置能力接成可编译、可测试的模块。
...
| `tests/net/epoller_test.cpp` | 修改 | 承载本 Step 对应代码、测试或文档变化 |
...
3. [Epoller::updateChannel()](../src/net/Epoller.cpp) 判断 fd 是否已经注册。
4. 新 fd 且有关注事件时执行 `EPOLL_CTL_ADD`。
5. 已注册 fd 且关注事件为空时执行 `EPOLL_CTL_DEL`。
6. 已注册 fd 且仍有关注事件时执行 `EPOLL_CTL_MOD`。
7. [Epoller::poll()](../src/net/Epoller.cpp) 调用 `epoll_wait()`。
...
- [validateChannelOwner()](../src/net/Epoller.cpp)：拒绝把其他 EventLoop 的 Channel 注册进来。
- [poll()](../src/net/Epoller.cpp)：处理 `EINTR`、填充 active_channels、必要时扩容事件数组。
- [updateChannel()](../src/net/Epoller.cpp)：统一处理 add/mod/del。
- [removeChannel()](../src/net/Epoller.cpp)：显式从 epoll 删除 Channel，并清空 `revents_`。
...
### updateChannel 的 add / mod / del 规则
...
| 风险 | 测试如何覆盖 |
| --- | --- |
| `实现 Epoller` 的核心契约只停留在接口说明里 | 用单元测试或集成测试固定 public API、正常路径和错误路径 |
...
我把 Linux epoll 封装成 Epoller，它负责创建 epoll fd、用 epoll_ctl 管理 fd 关注事件、用 epoll_wait 等待活跃事件，并通过 epoll_event.data.ptr 保存和取回 Channel。
