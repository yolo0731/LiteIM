...
Step 9 后只保留当前步骤真正需要的最小文件，不提前提交未来目录。
...
目标结构会按 Step 逐步创建，而不是在 Step 0 到 Step 9 一次性建立。
...
- 网络模块从 Step 7 开始放在 `include/liteim/net/` 和 `src/net/`，Step 7 加入 socket-agnostic `Buffer`，Step 8 加入 Linux socket 工具函数 `SocketUtil`，Step 9 加入 Reactor 核心接口 `Channel` / `Epoller` / `EventLoop`。
- 服务端入口放在 `server/`。
...
这些目录将在真正需要它们的 Step 中创建。当前 Step 9 只在 `net` 模块中补充 `Channel.hpp`、`Epoller.hpp`、`EventLoop.hpp` 和三个头文件编译测试，因为它只需要验证 Reactor 接口边界和头文件独立 include，不实现 `.cpp` 行为。
