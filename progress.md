...
本次进入 `Step 40: implement HeartbeatService heartbeat TTL refresh`。

开始状态：

- 当前新路线中 Step 39 `HistoryService` 已完成并提交：`6579df0 feat(chat): add recent history pagination`。
- `session-catchup.py` 提示的未同步内容来自旧的纯概念问答，不对应当前 Step 40 代码改动。
- 工作区进入 Step 40 前仍有用户侧未提交注释改动：`include/liteim/service/GroupService.hpp`、`src/service/GroupService.cpp`、`src/storage/GroupDao.cpp`。本 Step 不覆盖这些改动，提交时保持和 Step 40 行为改动分离。
- 用户确认采用方案 A：已登录心跳刷新 Redis TTL 失败时返回错误响应；未登录连接仍只返回 `HeartbeatResponse`。

概念计划：

- `HeartbeatService` 只负责处理业务层 `HeartbeatRequest`，返回 `HeartbeatResponse`。
- 未登录连接没有 user/session 绑定，handler 不写 Redis，直接返回成功。
- 已登录连接通过 `OnlineService::refreshUserOnline(user_id, session_id)` 刷新 Redis 在线 key TTL。
- 业务心跳不能直接更新 `Session::last_active_time`；完整、合法入站 Packet 的连接活跃语义仍由 `Session` 读路径负责。
- `MessageRouter` 的构造函数保留 inline heartbeat fallback；server runtime 注册 `HeartbeatService` 后覆盖为 business-thread handler。

TDD RED：

- 新增 `tests/service/heartbeat_service_test.cpp`，覆盖 header 自包含、未登录心跳成功且不刷新 Redis、已登录心跳刷新 Redis TTL、心跳不修改 `Session::last_active_time`、Redis 刷新失败返回错误、router 注册后发 `HeartbeatResponse`、router 注册后 Redis 失败发 `ErrorResponse`。
- 更新 `tests/CMakeLists.txt` 注册新测试文件。
- 运行 `cmake --build build`，预期失败于 `fatal error: liteim/service/HeartbeatService.hpp: No such file or directory`。

代码完成：

- 新增 `include/liteim/service/HeartbeatService.hpp` 和 `src/service/HeartbeatService.cpp`。
- `HeartbeatService` 注册 `HeartbeatRequest` 到 `MessageRouter` 的 business thread dispatch，覆盖 `MessageRouter` 构造函数里的 inline fallback 心跳 handler。
- 未登录连接通过 `OnlineService::getUserBySession(session_id)` 得到 `NotFound` 时返回 `HeartbeatResponse`，不写 Redis。
- 已登录连接先查到 `user_id`，再调用 `OnlineService::refreshUserOnline(user_id, session_id)` 刷新 Redis 在线 TTL。
- Redis 刷新失败按方案 A 返回错误状态，`MessageRouter` 会转换成 `ErrorResponse`。
- handler 不直接修改 `Session::last_active_time`。
- 更新 `src/service/CMakeLists.txt` 和 `server/main.cpp`，把 `HeartbeatService` 编进 `liteim_service` 并在运行时注册。

TDD GREEN：

- `cmake --build build`：通过。
- `ctest --test-dir build -R HeartbeatService --output-on-failure`：通过，7/7 tests passed。
- `ctest --test-dir build --output-on-failure`：通过，328/328 tests passed。
- `git diff --check`：通过。
- `timeout 1s ./build/server/liteim_server || test $? -eq 124`：通过，server 成功监听 `0.0.0.0:9000`，随后收到 SIGTERM 并正常退出。

完成收尾：

- 补齐 `README.md`、`task_plan.md`、`progress.md`，并新增 `tutorials/step40_heartbeat_service.md`。
- 本次 Step 40 已完成，剩余动作是提交当前 Step 改动并保留进入 Step 40 前就存在的 GroupService / GroupDao 用户注释改动与本次提交分离。
