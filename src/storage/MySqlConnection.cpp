...
Status MySqlConnection::connect(const MySqlConfig& config) {
    ...
    handle_ = mysql_init(
        nullptr);  // 创建/初始化一个 MYSQL 原生连接句柄，nullptr表示由目前没有 MYSQL 对象
    ...
    MYSQL* connected = mysql_real_connect(handle_, config.host.c_str(), config.user.c_str(),
...
Status PreparedStatement::prepare(const std::string& sql) {
    ...
    if (mysql_stmt_prepare(impl_->statement, sql.c_str(), static_cast<unsigned long>(sql.size())) !=
        0) {
...
//用于执行不会返回结果集的 SQL 语句，例如 INSERT、UPDATE、DELETE 等更新操作,执行成功后,通过 affected_rows 参数返回受影响的行数
Status PreparedStatement::executeUpdate(std::uint64_t& affected_rows) {
    affected_rows = 0;
    const auto bind_status =
        bindParameters();  //把之前 bindInt64() / bindString() 绑定好的参数真正交给 MySQL
    if (!bind_status.isOk()) {
        return bind_status;
    }
...
// 执行已经 prepare() 好的 SQL 语句，用于执行会返回结果集 的 SQL,例如 SELECT 查询语句，查询结果 rows/columns 存进 MySqlQueryResult
Status PreparedStatement::executeQuery(MySqlQueryResult& result) {
    ...
    if (metadata == nullptr) {
        ...
        return Status::error(ErrorCode::InvalidArgument,
                             "prepared statement did not return a result set");
    }
    const auto metadata_guard =
        std::unique_ptr<MYSQL_RES, decltype(&mysql_free_result)>(metadata, mysql_free_result);
    ...
    if (!result_binds.empty() &&
        mysql_stmt_bind_result(impl_->statement, result_binds.data()) != 0) {
        mysql_stmt_free_result(impl_->statement);
...
Status PreparedStatement::bindParameters() {
    ...
    if (!impl_->parameters.empty() &&
        mysql_stmt_bind_param(impl_->statement, impl_->parameters.data()) != 0)
