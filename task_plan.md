...
| Phase | Status | Notes |
| --- | --- | --- |
| Step 40 HeartbeatService Redis TTL refresh | done | Added `HeartbeatService` for `HeartbeatRequest` / `HeartbeatResponse`, kept TCP activity owned by the `Session` inbound packet path, returned success for unauthenticated heartbeat without Redis writes, refreshed Redis online TTL for authenticated sessions through `OnlineService::refreshUserOnline()`, and used scheme A for Redis refresh failure: return an error response instead of hiding the failure. No direct `Session::last_active_time` mutation, no timerfd/TcpServer heartbeat timeout changes, no reconnect policy, no client UI, no BotGateway. |
