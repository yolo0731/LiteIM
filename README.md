...
当前处于 `Step 7: add buffer abstraction`。

Step 7 的目标是进入网络模块，先实现 socket-agnostic `Buffer`，为后续 `Session` 输入缓冲区、输出缓冲区和慢客户端回压打基础。
...
- `tests/net/` 新增 13 个 `BufferTest` GoogleTest case；当前 CTest 共 67 个测试。

本 Step 只做内存字节缓冲区，不读 socket、不调用 epoll、不创建 Reactor 或 `Session`。网络 socket、epoll、Reactor 仍在后续 Step。仍然不提前创建 `client_cli/`、`client_qt/`、`bench/`、`scripts/`、`docker/` 等后续目录。

下一步是 `Step 8: implement SocketUtil`。
...
后续目录按 Step 逐步创建。比如 `SocketUtil`、`Epoller`、`Channel`、`EventLoop`、`Session`、CLI、Qt、benchmark、Docker 会在后续 Step 需要时创建。

## Step 7 验证

Step 7 有一个 server target、`liteim_base` / `liteim_protocol` / `liteim_net` library target 和一个 GoogleTest test target。运行：
...
CTest 应发现并通过 67 个测试。Step 7 新增测试：
