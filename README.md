...
当前处于 `Step 14: implement Session`。

Step 14 的目标是在 `liteim_net` 中实现单连接 owner `Session`：它接管一个已连接 fd，持有对应 `Channel`、输入/输出 `Buffer` 和 `FrameDecoder`，在 I/O loop 中循环读到 `EAGAIN`，把完整 `Packet` 交给 message callback，并把 `sendPacket()` 编码后的字节写入输出缓冲区，由 `EPOLLOUT` 继续发送。
...
- `tests/` 新增 Logger、EventLoop、Channel、Acceptor hardening round 2/3 回归测试和 Session 单连接 I/O 测试；当前 CTest 共 124 个测试。

本 Step 只实现单连接读写和 Packet I/O，不创建 `TcpServer`、`EventLoopThreadPool`，也不进入业务线程池、MySQL 或 Redis。仍然不提前创建 `client_cli/`、`client_qt/`、`bench/`、`scripts/`、`docker/` 等后续目录。

下一步是 `Step 15: implement EventLoopThread and EventLoopThreadPool`。
...
Step 14 有一个 server target、`liteim_base` / `liteim_protocol` / `liteim_net` library target 和一个 GoogleTest test target。运行：
...
CTest 应发现并通过 124 个测试。Step 14 新增测试：
