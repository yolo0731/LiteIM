...
当前处于 `Step 8: implement SocketUtil`。

Step 8 的目标是在网络模块里封装 Linux socket 常用函数，为后续 `Acceptor`、`Session` 和 `TcpServer` 统一创建非阻塞 fd、设置 socket option、查询 socket 错误和关闭 fd。
...
- `tests/net/` 新增 6 个 `SocketUtilTest` GoogleTest case；当前 CTest 共 73 个测试。

本 Step 只做 socket 工具函数，不调用 epoll、不创建 Reactor、`Acceptor`、`Session` 或 `TcpServer`。仍然不提前创建 `client_cli/`、`client_qt/`、`bench/`、`scripts/`、`docker/` 等后续目录。

下一步是 `Step 9: define Epoller / Channel / EventLoop interface`。
...
后续目录按 Step 逐步创建。比如 `Epoller`、`Channel`、`EventLoop`、`Session`、CLI、Qt、benchmark、Docker 会在后续 Step 需要时创建。

## Step 8 验证

Step 8 有一个 server target、`liteim_base` / `liteim_protocol` / `liteim_net` library target 和一个 GoogleTest test target。运行：
...
CTest 应发现并通过 73 个测试。Step 8 新增测试：
