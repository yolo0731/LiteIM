...
当前处于 `Step 12: implement EventLoop + eventfd task dispatch`。

Step 12 的目标是在 `liteim_net` 中实现真正的 `EventLoop` 事件循环，并用 `eventfd` 支持跨线程任务投递，让后续业务线程可以通过 `runInLoop()` / `queueInLoop()` 把连接操作送回所属 I/O 线程。
...
- `tests/net/` 新增 4 个 `EventLoopTest` 行为测试；当前 CTest 共 92 个测试。

本 Step 只实现 `EventLoop` 事件循环和 `eventfd` 任务投递，不创建 `Acceptor`、`Session` 或 `TcpServer`，也不进入业务线程池、MySQL 或 Redis。仍然不提前创建 `client_cli/`、`client_qt/`、`bench/`、`scripts/`、`docker/` 等后续目录。

下一步是 `Step 13: implement Acceptor`。
...
后续目录按 Step 逐步创建。比如 `Acceptor`、`Session`、CLI、Qt、benchmark、Docker 会在后续 Step 需要时创建。

## Step 12 验证

Step 12 有一个 server target、`liteim_base` / `liteim_protocol` / `liteim_net` library target 和一个 GoogleTest test target。运行：
...
CTest 应发现并通过 92 个测试。Step 12 新增测试：
