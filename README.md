...
当前处于 `Step 5: implement TlvCodec`。

Step 5 的目标是在 Step 4 Packet 外层结构之后，实现 body 内部 TLV 字段编解码，为后续登录、私聊、群聊、CLI/Qt/Python BotClient 复用同一套协议打基础。
...
本 Step 只做 TLV body 字段编解码。TCP 半包 / 粘包处理在 Step 6 实现，网络 socket、epoll、Reactor 仍在后续 Step。仍然不提前创建 `client_cli/`、`client_qt/`、`bench/`、`scripts/`、`docker/` 等后续目录。

下一步是 `Step 6: implement FrameDecoder`。
...
Step 5 有一个 server target、`liteim_base` / `liteim_protocol` library target 和一个 GoogleTest test target。运行：
...
CTest 应发现并通过 45 个测试。Step 5 新增测试：
