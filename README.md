...
当前处于 `Step 10: implement Epoller`。

Step 10 的目标是在 `liteim_net` 中实现真实 Linux `epoll` 封装，让 `Epoller` 可以创建 epoll fd、注册/修改/删除 `Channel`，并通过 `epoll_wait()` 返回活跃 fd 事件。
...
- 新增 `Channel.cpp`，实现构造、fd/event/revent 访问、事件 mask 修改和回调 setter。
...
- `tests/net/` 新增 5 个 `EpollerTest` 行为测试；当前 CTest 共 81 个测试。

本 Step 只实现 `Epoller` 和测试所需的最小 `Channel` 状态函数，不实现 `Channel::handleEvent()` 回调分发、不实现 `Channel::update()` 自动投递、不新增 `EventLoop.cpp`，也不创建 `Acceptor`、`Session` 或 `TcpServer`。仍然不提前创建 `client_cli/`、`client_qt/`、`bench/`、`scripts/`、`docker/` 等后续目录。

下一步是 `Step 11: implement Channel`。
...
后续目录按 Step 逐步创建。比如 `Channel::handleEvent()`、`EventLoop` 的 `.cpp` 实现、`Session`、CLI、Qt、benchmark、Docker 会在后续 Step 需要时创建。

## Step 10 验证

Step 10 有一个 server target、`liteim_base` / `liteim_protocol` / `liteim_net` library target 和一个 GoogleTest test target。运行：
...
CTest 应发现并通过 81 个测试。Step 10 新增测试：
