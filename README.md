...
当前处于 `Step 9: define Epoller / Channel / EventLoop interface`。

Step 9 的目标是先定义 Reactor 核心接口，让 `Channel`、`Epoller` 和 `EventLoop` 的职责边界清楚，再进入后续真实 `epoll` 实现和事件循环实现。
...
- 新增 `Epoller.hpp`，声明 `poll()`、`updateChannel()` 和 `removeChannel()`，并预留 epoll fd、事件数组和 fd 到 `Channel*` 的映射。
- 新增 `EventLoop.hpp`，声明 `loop()`、`quit()`、`updateChannel()`、`removeChannel()` 和线程归属检查。
- `tests/net/` 新增 3 个 `ReactorInterfaceTest` 编译测试；当前 CTest 共 76 个测试。

本 Step 只定义 Reactor 核心接口，不新增 `Epoller.cpp`、`Channel.cpp`、`EventLoop.cpp`，不调用 `epoll_create1()`、`epoll_ctl()`、`epoll_wait()`，也不创建 `Acceptor`、`Session` 或 `TcpServer`。仍然不提前创建 `client_cli/`、`client_qt/`、`bench/`、`scripts/`、`docker/` 等后续目录。

下一步是 `Step 10: implement Epoller`。
...
后续目录按 Step 逐步创建。比如 `Epoller`、`Channel`、`EventLoop` 的 `.cpp` 实现、`Session`、CLI、Qt、benchmark、Docker 会在后续 Step 需要时创建。

## Step 9 验证

Step 9 有一个 server target、`liteim_base` / `liteim_protocol` / `liteim_net` library target 和一个 GoogleTest test target。运行：
...
CTest 应发现并通过 76 个测试。Step 9 新增测试：
