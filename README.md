...
当前处于 `Step 6: implement FrameDecoder`。

Step 6 的目标是在 Packet 和 TLV 编解码之后，实现 TCP 字节流解包器，解决半包、粘包和错误 header 后的连接关闭决策问题。

本 Step 已完成：

- 新增 `FrameDecoder`，把连续 TCP 字节流解析成完整 `Packet`。
- `FrameDecoder::feed()` 内部持有未完成字节缓冲，支持一个包分多次输入。
- 支持一次输入多个完整包，也支持半包后接粘包。
- 错误 `magic`、`version`、`body_len` 超限时进入 error 状态。
- error 状态下拒绝继续解析，等待上层关闭连接或显式 `reset()`。
- `tests/protocol/` 新增 9 个 `FrameDecoderTest` GoogleTest case；当前 CTest 共 54 个测试。

本 Step 只做字节流到 Packet 的解包，不读 socket、不调用 epoll、不解析 TLV body、不做业务路由。网络 socket、epoll、Reactor 仍在后续 Step。仍然不提前创建 `client_cli/`、`client_qt/`、`bench/`、`scripts/`、`docker/` 等后续目录。

下一步是 `Step 7: implement Buffer`。
...
Step 6 有一个 server target、`liteim_base` / `liteim_protocol` library target 和一个 GoogleTest test target。运行：
...
CTest 应发现并通过 54 个测试。Step 6 新增测试：
