...
- `liteim_storage`: storage DTOs, the `IStorage` interface, `MySqlConnection`, `PreparedStatement`, `MySqlQueryResult`, `MySqlPool`, `ConnectionGuard`, `UserDao`, and `AuthDao` for future MySQL-backed users, friends, groups, messages, offline messages, and history.
