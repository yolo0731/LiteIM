...
当前处于 `Step 11: implement Channel`。

Step 11 的目标是在 `liteim_net` 中实现 `Channel` 事件分发，让 `Epoller::poll()` 写入的 `revents` 能被转换成 read/write/close/error 回调，并让关注事件变化通过所属 `EventLoop` 更新 epoll 注册状态。
...
- 新增 `EventLoop.hpp`，声明 `loop()`、`quit()`、`updateChannel()`、`removeChannel()` 和线程归属检查。
...
- 新增 `EventLoop.cpp`，只提供 `updateChannel()` / `removeChannel()` 到 `Epoller` 的最小桥接和线程归属检查，不实现阻塞事件循环。
- `Epoller` 使用 LT 模式，不设置 `EPOLLET`。
- `epoll_wait()` 返回时通过 `epoll_event.data.ptr` 找回 `Channel*`，并写入 `Channel::setRevents()`。
- `tests/net/` 新增 7 个 `ChannelTest` 行为测试；当前 CTest 共 88 个测试。

本 Step 只实现 `Channel` 事件分发和 `EventLoop` 更新桥接，不实现 `EventLoop::loop()` 阻塞循环、不实现 `eventfd` 任务投递，也不创建 `Acceptor`、`Session` 或 `TcpServer`。仍然不提前创建 `client_cli/`、`client_qt/`、`bench/`、`scripts/`、`docker/` 等后续目录。

下一步是 `Step 12: implement EventLoop + eventfd task dispatch`。
...
后续目录按 Step 逐步创建。比如 `EventLoop::loop()`、`eventfd` 任务投递、`Session`、CLI、Qt、benchmark、Docker 会在后续 Step 需要时创建。

## Step 11 验证

Step 11 有一个 server target、`liteim_base` / `liteim_protocol` / `liteim_net` library target 和一个 GoogleTest test target。运行：
...
CTest 应发现并通过 88 个测试。Step 11 新增测试：
