...
当前处于 `Step 16: implement TcpServer multi-Reactor version` 已完成状态。

Step 16 的目标是在 `liteim_net` 中实现多 Reactor `TcpServer`：主 `EventLoop` 持有 `Acceptor`，新连接到来后通过 `EventLoopThreadPool` round-robin 分配到子 I/O loop，并在对应 loop 中创建 `Session`。第一版业务只做 echo，用来验证网络底座、连接生命周期和跨线程发送路径。
...
- `tests/` 新增 Logger、EventLoop、Channel、Acceptor hardening round 2/3 回归测试、Session 单连接 I/O 测试、EventLoopThreadPool 多 Reactor 基础测试、Step 16 前 ByteOrder / Epoller owner-loop / Acceptor UniqueFd callback 清理回归测试，以及 Step 16 `TcpServer` 多 Reactor 回归测试；当前 CTest 共 142 个测试。

本 Step 只实现多 Reactor echo server 的网络层协调，不做业务线程池、MySQL、Redis、登录态、用户路由或慢客户端高水位策略。仍然不提前创建 `client_cli/`、`client_qt/`、`bench/`、`scripts/`、`docker/` 等后续目录。

下一步是 `Step 17: implement business ThreadPool`。
