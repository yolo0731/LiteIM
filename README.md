...
当前处于 `Step 18: implement TimerManager + timerfd heartbeat timeout` 已完成状态。

Step 18 的目标是在 `liteim/timer` 中补齐 `TimerHeap` 和 `TimerManager`，用 Linux `timerfd` 驱动服务端 idle session 清理。当前 `TcpServer` 在 base `EventLoop` 上注册一个 `TimerManager`，默认每 5 秒检查一次连接，90 秒未收到完整 `Packet` 的 `Session` 会通过 `Session::close()` 回到所属 I/O loop 关闭。第一版不实现登录心跳包、`HeartbeatService`、Redis TTL、用户在线状态或客户端重连。
...
- 新增 `TimerHeap.hpp` / `TimerHeap.cpp`，使用 `std::priority_queue` 小根堆保存 one-shot timer，返回自增 `TimerId`，`cancel()` 只标记取消，过期弹出时通过 lazy deletion 清理取消或陈旧条目。
- 新增 `TimerManager.hpp` / `TimerManager.cpp`，使用 `timerfd_create(CLOCK_MONOTONIC, TFD_NONBLOCK | TFD_CLOEXEC)` 创建定时 fd，用 `Channel` 注册到 owner `EventLoop`，每次 tick 读取 timerfd 并执行已过期 timer 回调。
- `TcpServer::setHeartbeatOptions()` 允许测试或后续配置在启动前设置心跳检查间隔和 idle 超时时间；生产默认仍是 5 秒 tick、90 秒 idle timeout。
- `TcpServer` 当前把 `TimerManager` 注册在 base loop 上，扫描线程安全 session 快照；真正关闭连接仍通过 `Session::close()` 投递回连接所属 I/O loop。
- `Session` 收到完整 `Packet` 时刷新 `lastActiveTimeMilliseconds()`，半包字节不会单独续期连接。
...
- `tests/` 新增 Logger、EventLoop、Channel、Acceptor hardening round 2/3 回归测试、Session 单连接 I/O 测试、EventLoopThreadPool 多 Reactor 基础测试、Step 16 前 ByteOrder / Epoller owner-loop / Acceptor UniqueFd callback 清理回归测试、Step 16 `TcpServer` 多 Reactor 回归测试、Step 17 `ThreadPool` 业务线程池测试、Step 18 `TimerHeap` / `TimerManager` 测试和 idle/active session heartbeat 回归测试；当前 CTest 共 164 个测试。

本 Step 只实现 timerfd 驱动的服务端 idle 连接清理，不接入 MySQL、Redis、登录态、用户路由、MessageRouter 或客户端重连。慢客户端输出缓冲 4MB 超限关闭已补齐，完整业务级限流/降级策略仍留给后续业务 Step。仍然不提前创建 `client_cli/`、`client_qt/`、`bench/`、`scripts/`、`docker/` 等后续目录。
...
当前有一个 server target、`liteim_base` / `liteim_protocol` / `liteim_net` / `liteim_concurrency` library target 和一个 GoogleTest test target。运行：
...
CTest 应发现并通过 164 个测试。Step 18 新增测试：

- `TimerInterfaceTest.TimerHeapHeaderIsSelfContained`
- `TimerInterfaceTest.TimerManagerHeaderIsSelfContained`
- `TimerHeapTest.PopExpiredRunsCallbacksInDeadlineOrder`
- `TimerHeapTest.CancelUsesLazyDeletionWithoutRemovingNewTimer`
- `TimerHeapTest.PopExpiredIgnoresFutureTimers`
- `TimerManagerTest.TimerFdTickRunsExpiredTimer`
- `TimerManagerTest.CancelledTimerDoesNotRun`
- `TcpServerTest.IdleSessionIsClosedByHeartbeatTimeout`
- `TcpServerTest.ActiveSessionSurvivesHeartbeatTimeout`
...
Step 15: implement EventLoopThread and EventLoopThreadPool
Step 16: implement TcpServer multi-Reactor version
```
...
- [`tutorials/step15_event_loop_thread_pool.md`](tutorials/step15_event_loop_thread_pool.md)：Step 15 教程。
- [`tutorials/step16_tcp_server.md`](tutorials/step16_tcp_server.md)：Step 16 教程。
