...
当前处于 `Step 3: define MessageType and TLV types`。

Step 3 的目标是在 Step 2 的基础模块之后，新增第一层协议类型定义，为后续 Packet 编解码、TCP 流式解码、CLI/Qt/Python BotClient 跨语言协议兼容打基础。

本 Step 已完成：

- 新增 `include/liteim/protocol/` 和 `src/protocol/`，编译生成 `liteim_protocol` 静态库。
- 新增 `MessageType`，覆盖心跳、注册登录、好友、私聊、群聊、离线消息、历史消息、Bot 消息和错误响应。
- 新增 `TlvType`，覆盖用户名、密码、用户 ID、好友 ID、群组 ID、会话 ID、消息内容、错误信息、Bot ID 和 Persona ID 等字段类型。
- 新增 `toString(MessageType)` 和 `toString(TlvType)`，让日志、测试和后续协议调试有稳定可读名称。
- 新增 `isRequestType()` 和 `isResponseType()`，为后续 `MessageRouter` 分发请求/响应打基础。
- `tests/protocol/` 新增 7 个 GoogleTest case；加上 Step 1 和 Step 2 测试，当前 CTest 共 22 个测试。

本 Step 只创建当前需要的 `include/liteim/protocol/`、`src/protocol/` 和 `tests/protocol/`。仍然不提前创建 `client_cli/`、`client_qt/`、`bench/`、`scripts/`、`docker/` 等后续目录。

下一步是 `Step 4: implement Packet encode/decode`。
...
后续目录按 Step 逐步创建。比如 `include/liteim/net/` 和 `src/net/` 会在后续网络 Step 需要时创建。

## Step 3 验证

Step 3 有一个 server target、`liteim_base` / `liteim_protocol` library target 和一个 GoogleTest test target。运行：
...
- `MessageTypeTest.PushAndUnknownTypesAreNotRequestOrResponse`
