...
- `liteim_storage`: storage DTOs, the `IStorage` interface, `MySqlConnection`, `PreparedStatement`, `MySqlQueryResult`, `MySqlPool`, `ConnectionGuard`, `UserDao`, `MessageDao`, `OfflineMessageDao`, `FriendDao`, `GroupDao`, and `MySqlStorage` for MySQL-backed users, friend requests, accepted friend profiles, groups, messages, offline messages, delivery states, and history.
...
Private-message retry idempotency is optional and backward compatible. A newer client may include `ClientMessageId` in `PrivateMessageRequest`; LiteIM stores it as `messages.client_msg_id` and enforces uniqueness on `(sender_id, client_msg_id)`. If the same sender retries the same id after a network timeout, the server returns the existing stored message fields instead of inserting a duplicate. The retry path also re-checks the receiver delivery side: already delivered/read messages are not re-pushed, pending/missing delivery state may be repaired with online push or an offline fallback row, and an already existing offline row is accepted idempotently without double unread. Older clients that do not send `ClientMessageId` keep the original non-idempotent send behavior.
...
| `message_deliveries` | per-user delivery state linked to `messages`; current implementation writes pending and delivered, with pushed/read states reserved for future richer delivery tracking |
...
- The current delivery model demonstrates server-stored ACK, offline ACK, private delivery ACK, synchronous online-push fallback, and sender retry idempotency, but it is not a full multi-device delivery state machine. A packet that was accepted into an I/O loop and then lost before receiver ACK still depends on sender retry or future delivery-state repair; `device_id` is not part of the current protocol.
