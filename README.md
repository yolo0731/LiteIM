...
当前处于 `Step 4: implement Packet encode/decode`。

Step 4 的目标是在 Step 3 协议类型定义之后，实现固定 Packet header 的编码、解析和校验，为后续 TLV body 编解码、TCP 流式解码、CLI/Qt/Python BotClient 跨语言协议兼容打基础。

本 Step 已完成：

- 新增 `PacketHeader`，固定 20 字节 header：`magic`、`version`、`flags`、`msg_type`、`seq_id`、`body_len`。
- 新增 `Packet`，由 `PacketHeader` 和原始 `body` 字节组成。
- 新增 `validateHeader()`，校验 `magic == 0x4C494D31`、`version == 1`、`body_len <= 1MB`。
- 新增 `encodePacket()`，按网络字节序写入 header，并把 `body.size()` 作为真实 `body_len`。
- 新增 `parseHeader()`，从字节数组解析 fixed header，并复用 `validateHeader()` 返回明确错误。
- `tests/protocol/` 新增 10 个 `PacketTest` GoogleTest case；当前 CTest 共 33 个测试。

本 Step 只做 Packet header 和原始 body 字节拼接。TLV body 的字段编码在 Step 5 实现，TCP 半包 / 粘包处理在 Step 6 实现。仍然不提前创建 `client_cli/`、`client_qt/`、`bench/`、`scripts/`、`docker/` 等后续目录。

下一步是 `Step 5: implement TlvCodec`。
...
Step 4 有一个 server target、`liteim_base` / `liteim_protocol` library target 和一个 GoogleTest test target。运行：
...
CTest 应发现并通过 33 个测试。Step 4 新增测试：
