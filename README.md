...
当前处于 `Step 1: init CMake project structure`。

Step 1 的目标是把 Step 0 的空 CMake 骨架升级成最小可构建工程，并从一开始接入 GoogleTest。

本 Step 已完成：

- 根 `CMakeLists.txt` 接入 `server/` 和 `tests/`。
- 根 `CMakeLists.txt` 使用 `FetchContent` 拉取 GoogleTest v1.14.0。
- `server/` 编译生成 `liteim_server`。
- `tests/` 编译生成 `liteim_tests`，链接 `GTest::gtest_main`，并通过 `gtest_discover_tests` 注册到 CTest。
- `server/main.cpp` 暂时只打印启动信息。
- `tests/test_main.cpp` 使用 `TEST(SmokeTest, GoogleTestWorks)` 验证 GoogleTest 链路。

本 Step 不提前创建 `include/`、`src/`、`client_qt/`、`bench/`、`scripts/`、`docker/` 等后续目录。
这些目录会在真正需要它们的 Step 中创建。

下一步是 `Step 2: add config logger and error code`。
...
后续目录按 Step 逐步创建。比如 `include/liteim/base/` 和 `src/base/` 会在 Step 2 需要基础模块时创建。

## Step 1 验证

Step 1 有一个最小 server target 和一个 GoogleTest smoke test target。运行：
...
CTest 应发现并通过 `SmokeTest.GoogleTestWorks`。
